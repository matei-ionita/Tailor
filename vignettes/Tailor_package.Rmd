...
* learning a model of the data: `tailor.learn()`, `get_1D_mixtures()`, `customize_1D_mixtures()`, `categorical_labeling()`

* predicting the most likely cluster membership for a batch of data: `tailor.predict()`

* visualization: `inspect_1D_mixtures()`, `plot_tsne_clusters()`, `plot_tsne_global()`


## Default workflow
Some preparations: load the package, some data, choose the parameters to be used in the clustering analysis, and choose a seed for the random generator.

```{r preparations}
library(Tailor)
filename = system.file("extdata", "sampled_flowset_young.rda", package = "Tailor")
load(filename)   # fs_young

tailor_params = flowCore::colnames(fs_young)[c(7:9, 11:22)]
seed = 525
...
The simplest way to learn a Tailor model is to feed the data directly to the `tailor.learn()` method. (See the Advanced workflow section below for more complex options.)

```{r tailor_vanilla}
n_components = 50

tailor_obj = tailor.learn(data = fs_young,
...
Given the Tailor object computed with `tailor.learn()`, we can use `tailor.predict()` to assign cluster membership to data. In this example, we use the same data that was used in the learning step. But `tailor.predict()` can be used just as well with new data.

```{r tailor_predict}
tailor_pred = tailor.predict(data = fs_young,
                             tailor_obj = tailor_obj,
                             parallel = TRUE)
```

`tailor.predict()` returns two lists. `$mixture_mapping` assigns each datapoint to a mixture component. `$cluster_mapping` assigns each datapoint to a categorical cluster.
...
mixtures_1D = customize_1D_mixtures(fs_young, to_customize, mixtures_1D, verbose = TRUE)
inspect_1D_mixtures(fs_young, mixtures_1D, names(to_customize))
```

That looks better.
We can pass the 1D mixtures as an argument to `tailor.learn()`, and they will be used in the binning process. We can also specify some 1D mixture components that should be merged in the same bin, if we have grounds to believe that they represent the same biological population.
...
plot_tsne_global(fs_young, tailor_obj, tailor_pred, defs, seed = seed)
```
