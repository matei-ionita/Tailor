...
* Some mixture components are dropped at initialization stage, if they contain too few events. The user is warned when this happens.
...
```{r tailor_plot_mfi, fig.align= "center", fig.width=9, fig.height=7}
set.seed(525) # use the same seed as above, so figures can be compared
...
Most of the 1D models look reasonable. But say that we wanted to model CD127 as two mixture components instead of one. We can use the method `customize_1D_mixtures()` to update a user-specified list of parameters. Compare the result to the previous 1-component model for CD127 above.

```{r customize_1D_mixtures, fig.width=8, fig.height=4}
to_customize <- list("CD127BV421" = 2) # model CD127 with 2 mixture components
...
We can pass the 1D mixtures created above as an argument to `tailor_learn()`, and they will be used in the binning process. We can also specify some 1D mixture components that should be merged in the same bin, if we have grounds to believe that they represent the same biological population.
...
```{r tailor_plot_fluorescence_advanced, fig.align="center", fig.width=9, fig.height=7}
set.seed(525)
plot_tailor_fluorescence(tailor_obj)
```
