...
{ 
  ...
  # Suppose we treat the smallest i clusters as outliers 
  for (i in seq(l)) 
  { 
    ...
    { 
      ...
      { 
        offset = i-1 
        cvo = matrix(nrow = l-offset, ncol = 2) 
        colnames(cvo) = c("clusters", "outliers") 
        created = TRUE 
      } 
      
      cvo[i - offset,] = c(l-i+1, outliers) 
    } 
