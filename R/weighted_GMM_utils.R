...
#     steps: number of steps for the EM algorithm; haven't implemented termination
#             condition yet
#     regularize_variance: a boolean flag; if true, adds a small multiple of the
#             d x d identity matrix to each variance
#     variance_correction: variance lost in downsampling, added back at each iteration
...
#' @title bulk_weighted_gmm
#' @description This function calculates a bulk weighted gmm
#' @param data a matrix containing events along the rows, variables along columns
#' @param k the number of mixture components (clusters)
#' @param weights a vector of length = nrow(data), specifying the weight of each event
#' @return A gmm object
#' @export
bulk_weighted_gmm = function(data, k, params, weights = NULL,
                             initialize = NULL, seed = 91, steps = 10,
...
predict_bulk_gmm = function(data, mixture, params, n_batch = 64,
                            compute_loglik = TRUE, parallel = TRUE, verbose = FALSE)
{
  start.time = Sys.time()
  n = nrow(data)
  k = length(mixture$pro)
  logpro = log(mixture$pro)
  ...
  if (parallel)
  {
    ...
    mapping = foreach(batch_data=iter(data_list), .combine = c, .packages = c("mvtnorm")) %dopar%
    {
      ...
      for (cl in seq(k))
      {
        weight = mixture$pro[cl]
        mean = mixture$mean[cl,]
        sigma = mixture$variance$sigma[,,cl]
        # posteriors[,cl] = weight * dmvnorm(batch_data, mean, sigma, log = FALSE)
      ...
      #   probsum = apply(posteriors, 1, sum)
      #   log_likelihood = log_likelihood + sum(log(probsum))
      # }
    }
  ...
  if(compute_loglik) list("mapping" = mapping, "loglik" = log_likelihood)
  else list("mapping" = mapping)
}
...
# arising from replacing a bunch of events with the center of their phenobox
